...
class Quadcopter:
    ...
    def check_quadcopter_type(self):
        ...
        if self.quadcopter_type == QuadcopterType.QUADCOPTER:
            raise ValueError(f"You should choose a quadcopter type, not {self.quadcopter_type}")
    ...
    def _setup(self):
        ...
        self.propulsion_system = PropulsionSystem(self.id, self.model, self.droneSpecs, self.environment_parameters, self.use_direct_velocity)
    ...
    def _publish_flight_state(self):
        ...
        message = {
            **inertial_data,
            "publisher_type": self.quadcopter_type
        }
        
        self.messageHub.publish(topic="flight_state", message=message, publisher_id=self.id)
    ...
    def _compute_velocity_from_command(self, motion_command: np.ndarray) -> np.ndarray:
        ...
        return motion_command[3] * motion_command[:3] / norm
