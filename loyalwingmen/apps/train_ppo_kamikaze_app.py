...
if train_flag:
    env = Battlefield()
    #vectorized_environment = make_vec_env(Battlefield, n_envs=4)
    model = generate_model(env, nn_t=[512, 512, 512])
    train(
        model,
        env,
...
if test_flag:
    ...
    test(model, Battlefield)

# pegar o que esta no random_optimzer_pp.py e cpçpcar aqui tbm
# usar com SubprocVecEnv
